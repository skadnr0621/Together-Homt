...
public class FollowController {
    ...
    public ResponseEntity<?> listFollowers() {
        List<UserDto> followers = followService.listFollowers();
        return followers.size() == 0
                ? new ResponseEntity<>("나를 팔로우하는 사람이 없습니다.", HttpStatus.OK)
    ...
    public ResponseEntity<?> listFollowings() {
        List<UserDto> followings = followService.listFollowings();
        return followings.size() == 0
                ? new ResponseEntity<>("내가 팔로우하는 사람이 없습니다.", HttpStatus.OK)
